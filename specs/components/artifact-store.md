...
- The **gate verdict** (kind `gate-verdict`): the assembled, per-check result of one
  gate run — passed/failed/not-run, the per-check parsed [findings](../verification.md)
  (`file:line` + message), red→green base/candidate, mutation score vs. threshold,
  ...
  The bulky per-check raw *output* it references stays the separate gate-evidence entries
  above; this record holds the structured findings and indexes the raw output. See
