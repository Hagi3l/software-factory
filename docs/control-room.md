...
- **Live invocation** (`/invocation/{id}`) — watch *one* worker think: the agent's
  ...
  carrying its role/stage and a budget meter advancing toward the wall/token ceiling (with
  the token burn's in/out/cached breakdown beneath it). Rows produced by the
  [`explore`](../specs/components/agent.md) tool's nested read-only sub-loop are **nested**
  — indented, badged `explorer` — so its cheap-tier comprehension reads as sub-activity of
  the parent invocation rather than as parent turns. It
...
- **Verification** (`/verification/{id}`) — the factory's *trust argument* for one issue,
  ...
  tools — the **transformation log**: each `rename`/`code_action` tagged with the
...
4. **APPROVE** — an explicit consent gate, **soft-gated on a converged ledger**: you
   ...
   it commits the spec onto a fresh `epic/<epic_id>` branch cut from `main`
