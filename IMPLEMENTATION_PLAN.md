...
- [ ] **T2.15 Tests-red compiled-language complement: a `compiles` companion check** — `tests-red` reuses `tests-pass` and passes iff it exits **nonzero**, but in a compiled, statically-typed language (Go) a test referencing a not-yet-defined symbol **fails to compile**, which also exits nonzero — so a suite that never executed a single assertion satisfies `tests-red` (a *vacuous* red; `verification.md` had overclaimed it as proof of "executing" tests). It also lets an under-defined contract flow downstream, where the implementor invents the API surface itself instead of inheriting it. Fix is a **language-neutral companion check**, not a Go-aware gate: a target registers `compiles` (Go: `go build ./... && go test -run='^$' ./...` — builds the test binaries without running them) and lists **both** `tests-red` *and* `compiles` on the `author-tests` postcondition, so "red" means **compiles ∧ tests-fail**. No gate-engine change (`compiles` is an ordinary exit-0-pass check; postconditions already AND). Benefits: forces the test author to commit the minimal compiling API skeleton (interface ownership stays with the producer), and strengthens red→green (a provably-compiling base makes "fails on base" a failed assertion, not a build break). **Spec done:** [verification.md](specs/verification.md) "Tests-red proof" — overclaim fixed + the compiled-language complement documented; kernel stays language-neutral, opt-in per target. **Wiring is vault-demo-only** (shipped `config/harness.yaml` leaves `tests-red` unpaired): add a `compile` target to `demo/vault/app/Makefile`; register `compiles: make compile` + add it to the demo's `author-tests` postcondition (fail-fast, not in `independent_checks`); reconcile the demo test-author persona's "prefer to avoid scaffolding" → "commit the minimal compiling skeleton"; note it in `demo/vault/README.md`.
