...
3. **Slices, not layers — each child must stand up end-to-end on its own.** "Independently
   ...
   architectural layer** — a separate child for the schema, another for the store method,
   another for the HTTP handler, another for the view of *one* feature. Those layers are NOT
   independently testable: the view's tests need the handler, whose tests need the store, whose
   tests need the schema. The test author for a leaf layer (a view, a handler) is then forced to
   write whole-*feature* tests that exercise siblings which do not exist yet, and the implementor
   for that layer finds either nothing to build (a lower layer already supplied it) or the whole
   feature to build (out of its scope) — and it dead-letters. Split **vertically**: each child is
   a thin end-to-end capability that cuts through whatever layers it needs and is demonstrable
   through one seam (an HTTP request, a CLI command, a function call). *"Generate a one-time
   share link"* (token + store row + endpoint + minimal markup, provable with one httptest) is
   **one vertical child**; *"add the share_tokens table"*, *"add the store method"*, *"add the
   POST handler"*, *"add the templ component"* are **four layers of one child** that cannot stand
   alone. So: if a feature is small enough to build and verify in one pass, it is **one child** —
   do not slice it at all. Only split when you can name two or more capabilities that are *each*
   independently demonstrable end-to-end (e.g. "generate a share link" vs "reveal-and-burn a
   share link"), and wire their order with `depends_on`. This refines rule 2: "when in doubt,
   split" means split into **vertical slices**, never into layers.
