...
| no `explore` (shipped config sets no `policy.explore_budget`) | `explore_budget: { tokens: 100_000, turns: 12 }` + a `vault-explorer` soul on cheap Haiku, `explore` enabled on the planner + implementor | the `explore` tool delegates broad *search→read→refine* localization to a nested read-only sub-loop on a **cheap** model in the same warm sandbox, handing the frontier producer back distilled `file:line` anchors instead of making it navigate in its own (Opus/Sonnet) context — the planner (finding the seams to decompose) and implementor (finding where the store/web/view layers hook up) are the beneficiaries. The **verify path (`qa`/`security`) enables no explore**: it re-grades independently and reads raw, so its blind spots never correlate with the producer's (the stricter, fully-independent option in specs/verification.md "verify-path diversity"). The explorer runs on `anthropic/claude-haiku-4.5` — its output is *checkable* anchors an independently-gated parent re-reads, so a weak model there costs at worst a re-search, never a bad merge (specs/models.md "Helper souls") |
