...
1. Go to **`/create`** (the Create-Task wizard). Describe the feature you want. The
   ...
   and once intent has converged it drafts a **spec + seed issue(s)**. For a time-boxed
   stage run, the composer's **Insert prepared requirement** button
   (`requirements_planner.prefill` → [`config/prompts/share-link-request.md`](config/prompts/share-link-request.md))
   drops in a fully-specified share-link requirement that pre-resolves every design fork
   (told to ledger them as `agreed` and draft in one turn, seeding exactly one epic root) —
   you review it in the box and press Enter, skipping the back-and-forth without skipping
   the consent gate. Edit that file to change the kickoff; it is re-read on each page load.
...
| no `explore` (shipped config sets no `policy.explore_budget`) | `explore_budget: { tokens: 100_000, turns: 12 }` + a `vault-explorer` soul on cheap Haiku, `explore` enabled on the planner + test-author + implementor | the `explore` tool delegates broad *search→read→refine* localization to a nested read-only sub-loop on a **cheap** model in the same warm sandbox, handing the frontier producer back distilled `file:line` anchors instead of making it navigate in its own (Opus/Sonnet) context — all three producers benefit: the planner (finding the seams to decompose), the test-author (finding the seams its tests must pin — the demo's proven raw-navigation token hog, ~1.3M tokens in one live run), and the implementor (finding where the store/web/view layers hook up). The **verify path (`qa`/`security`) enables no explore**: it re-grades independently and reads raw, so its blind spots never correlate with the producer's (the stricter, fully-independent option in specs/verification.md "verify-path diversity"). The explorer runs on `anthropic/claude-haiku-4.5` — its output is *checkable* anchors an independently-gated parent re-reads, so a weak model there costs at worst a re-search, never a bad merge (specs/models.md "Helper souls") |
