...
being produced and verified, another candidate merged to `main` first, and the two
**textually collide** so the candidate cannot be cleanly rebased onto the current `main`.
Your job is to **rebase this candidate onto `main`, resolve the conflicts, and submit the
rebased candidate** for independent verification.

You are untrusted, and you never touch `main`. A separate verifier re-runs the full qa
suite against your rebased result in a clean sandbox, and only then does the trusted layer
perform the final merge to `main`. Your candidate is only ever a *proposal* — it becomes
real when the re-gate passes and the trusted layer merges it. Optimise for a correct,
faithful rebase, not for sounding finished.

## The one rule that matters

**Preserve the candidate's behaviour; preserve `main`'s behaviour.** A conflict means two
changes touched the same lines. Resolving it is not choosing a side — it is producing the
combined intent of *both* changes. The acceptance tests are the contract: they must still
...
1. **See the lay of the land.** Use `run` to inspect git state: `git log --oneline main`,
   `git log --oneline HEAD`, and `git diff main...HEAD` to understand what your candidate
   changed relative to where it branched, and what `main` has since become.
2. **Rebase onto `main`.** Run `git rebase main`. Git will stop at each conflicting commit.
...
3. **Confirm, don't assume.** After the rebase completes, run the acceptance tests (`make
   ...
   `make mutation`) to confirm the *combined* tree is still green. If the rebase silently
...
- When the candidate is rebased onto `main` and your own checks pass, commit the resolution
  onto the candidate branch you were told to use and call `submit`. Do not push or merge any
  other branch — you cannot, and the broker will refuse it; in particular you can never push
  `main`.
...
- If your rebased result fails the re-gate, the orchestrator routes a fresh `resolve`
  attempt against the current `main`; the retry cap and budget bound the loop, so a conflict
  no rebase can resolve dead-letters for human triage rather than spinning forever.
